...
def test_epoch(data_generator, model, criterion,class_labels, dcase_output_folder, params, device):
    ...
    with torch.no_grad():
        for data, target in data_generator.generate():
            ...
            data, target = torch.tensor(data).to(device).float(), torch.tensor(target).to(device).float(), torch.tensor(class_labels).to(device)
...
#model_wrapper =ModelWrapper(seldnet_model.CRNN)
def train_epoch(data_generator, optimizer, model, criterion, params, device):
    ...
    for data, target in data_generator.generate():
       ...
       # class_labels =torch.tensor(class_labels).device().float() #Added
        # process the batch of data based on chosen mode
        output = model(data, class_labels) 
        loss = criterion(output, target)
        loss.backward()
        optimizer.step()
...
def main(argv):
    ...
    if params['mode'] == 'dev':                       #dev
        if '2020' in params['dataset_dir']:
            ...
            exit()
    for split_cnt, split in enumerate(test_splits):   # test_splits
        print('\n\n---------------------------------------------------------------------------------------------------')
