...
class FiLM(nn.Module):
    def __init__(self, gamma_size, beta_size):
        super(FiLM, self).__init__()
        self.embedding = nn.Embedding(256, 128)  
        self.linear_gamma = nn.Linear(128, gamma_size)
        self.linear_beta = nn.Linear(128, beta_size)
        self.dropout = nn.Dropout(p=0.05)

    def forward(self, class_labels):
        embedded = self.embedding(class_labels)
        embedded = torch.mean(embedded, dim=1)  
        embedded = self.dropout(embedded)
        gamma = F.relu(self.linear_gamma(embedded))  
        beta = F.relu(self.linear_beta(embedded))  
        embedded = self.dropout(embedded)
        return gamma, beta

class FiLMGenerator(nn.Module):
    def __init__(self, num_categories, output_size):
        super(FiLMGenerator, self).__init__()
        self.num_categories = num_categories
        self.output_size = output_size
        self.embedding = nn.Embedding(num_categories, output_size * 2)  #  doubling the output size to generate both gamma and beta

    def forward(self, x):
       
        x_one_hot = F.one_hot(x, num_classes=self.num_categories).float()  
        # Embed the one-hot encoding
        embedded = self.embedding(x_one_hot)  # Use x_one_hot 
        
        # Split the output into gamma and beta
        gamma, beta = torch.chunk(embedded, 2, dim=2)  
        return gamma, beta

class CRNN(torch.nn.Module):
    def __init__(self, in_feat_shape, out_shape, params, num_categories= params['unique_classes']):
        super().__init__()
        self.nb_classes = params['unique_classes']
        self.conv_block_list = torch.nn.ModuleList()
        self.film_generator = FiLM(params['unique_classes'], params['nb_cnn2d_filt'])
        self.film_generator = FiLMGenerator(num_categories, len(params['f_pool_size']) * params['nb_cnn2d_filt'])
        self.spatial_fc = nn.Linear(2, params['unique_classes']) 
        ...
        self.film_layers = nn.ModuleList([
            FiLM(params['num_categories'], params['nb_cnn2d_filt']) for _ in range(len(params['f_pool_size']))
        ])
    ...
    def forward(self, x, _sed_labels):
        gammas, betas = self.film_generator(_sed_labels)
        film_index = 0
        for conv_cnt in range(0, len(self.conv_block_list), 3):
            x = self.conv_block_list[conv_cnt](x)
            x = self.conv_block_list[conv_cnt + 1](x)
            x = self.conv_block_list[conv_cnt + 2](x)
            gamma_block, beta_block = self.film_layers[film_index](_sed_labels)
            gamma_block = gamma_block.unsqueeze(-1).unsqueeze(-1)
            beta_block = beta_block.unsqueeze(-1).unsqueeze(-1)
            x = x * gamma_block + beta_block
            film_index += 1
