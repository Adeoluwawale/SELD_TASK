...
from transfercnn import ConvBlock , DoubleConv
...
def one_hot_encode(class_labels, params ):
    
    return F.one_hot(class_labels, num_classes=params['unique_classes']).float()
class ControlUnit(nn.Module):
    ...
    def forward(self, question):
        control = F.relu(self.fc(question))
        return control
...
class ReadUnit(nn.Module):
    ...
    def forward(self, context, control):
        context = context * control.unsqueeze(2)
        read = F.relu(self.fc(context))
        return read
...
class WriteUnit(nn.Module):
    ...
    def forward(self, memory, read):
        memory = F.relu(self.fc(memory + read))
        return memory
...
class MACCell(nn.Module):
    ...
    def forward(self, question, context, memory):
        control = self.control(question)
        read = self.read(context, control)
        memory = self.write(memory, read)
        return memory

class MACNetwork(nn.Module):
    def __init__(self, dim, num_layers):
        super(MACNetwork, self).__init__()
        self.mac_cells = nn.ModuleList([MACCell(dim) for _ in range(num_layers)])
        self.fc_output = nn.Linear(dim, dim)  # Example output layer

    def forward(self, question, context):
        memory = torch.zeros_like(question)
        for cell in self.mac_cells:
            memory = cell(question, context, memory)
        output = self.fc_output(memory)
        return output

class ConvBlock(nn.Module):
    def __init__(self, in_channels, out_channels, pool_size, dropout_rate):
...
class CRNN(torch.nn.Module):
    def __init__(self, in_feat_shape, out_shape,params, num_mac_layers=4):
        super().__init__()
        self.nb_classes = params['unique_classes']
        self.conv_block_list = torch.nn.ModuleList()
        # Define the CNN layers with MAC integration
        for conv_cnt in range(len(params['f_pool_size'])):
            ...
            self.conv_block_list.append(MACNetwork( params['nb_cnn2d_filt'],params['unique_classes']))
            self.conv_block_list.append(
       ...
       # self.mac = MACNetwork(params['nb_cnn2d_filt'], num_mac_layers)
...
#            self.attn = AttentionLayer(params['rnn_size'], params['rnn_size'], params['rnn_size'])
    ...
    def forward(self, x,class_labels):  # class-labelsparams['unique_classes']
        '''input: (batch_size, mic_channels, time_steps, mel_bins)'''
        #question = one_hot_encode(class_labels, num_classes=params['unique_classes'])
        #class_labels_one_hot = F.one_hot(class_labels, params= ['unique_classes']).float() # num_classes = params['unique_classes']

       # class_labels_one_hot = one_hot_encode(class_labels, params=['unique_classes'])
        
        for i in range(len(self.conv_block_list)):
            x = self.conv_block_list[i](x)
        class_labels_one_hot = F.one_hot(class_labels, num_classes=self.num_classes).float()

        for i in range(len(self.conv_block_list)):
            x = self.conv_block_list[i](x)
            if i % 3 == 0 and self.num_mac_layers > 0:  # Apply MAC layer after each ConvBlock
                x = self.mac_layers[i // 3](x, class_labels_one_hot)
