...
from transfercnn import  ConvBlock, Cnn14  #Cnn14_DecisionLevelAtt,
...
#            self.attn = AttentionLayer(params['rnn_size'], params['rnn_size'], params['rnn_size'])
        ...
        self.pretrained2crnn = nn.Linear(2048, 128)
        self.merge2crnn = nn.Linear(256, 128)
   
   
    def forward(self, x,):
        '''input: (batch_size, mic_channels, time_steps, mel_bins)'''

        pretrained_x = self.basemodel.forward(x)
         #obtain the embedding only because we dont care about the classifiers
        pretrained_x = pretrained_x["embedding"]
        #   print the shape of the pretrained x and x itself.
       
        print("Pretrained shape:", pretrained_x.shape, "Input shape:", x.shape)
         #Instead, applying a linear to transform the feature to the desired shape.
        # pretrained_x = pretrained_x.mean(dim=1, keepdim=True)
        pretrained_x = self.pretrained2crnn(pretrained_x)
        
        print("Pretrained shape after linear:", pretrained_x.shape)
        # concatenation of the pretrained feature with the input feature.
        
        x = torch.cat([x, pretrained_x.unsqueeze(1).unsqueeze(1).repeat(1, x.shape[1], x.shape[2], 1)], dim=-1)
        print("Input shape after concatenation:", x.shape)
        #  concatenation of the global feature to each frames. So we use another linear to project the dimension back
        x = self.merge2crnn(x)
        print("Input shape after merging:", x.shape)
        #  here the shape of x is the same as the module input:  (batch_size, mic_channels, time_steps, mel_bins)
        # 
