...
class DoubleConv(nn.Module):
        def __init__(self, in_channels, out_channels, 
            ...
            self.init_weights()
            num_channels = self.double_conv[0].in_channels
           # Split the convolution block into two equal parts
            self.conv1 = nn.Sequential(*self.double_conv[:num_channels // 2])
            self.conv2 = nn.Sequential(*self.double_conv[num_channels // 2:])
        ...
        def forward(self, x):
           ...
           # Apply the first sub-block
            out1 = self.conv1(x)
        
        # Apply the second sub-block
            out2 = self.conv2(x)
            return out1, out2

            #return x
        def orthogonalize(self):
        # Orthogonalize the weight matrices of the two subspaces
            for layer1, layer2 in zip(self.conv1, self.conv2):
                ...
                Q, _ = torch.qr(weight1)
                weight2.data = Q.clone()
