...
from methods.utils.model_utilities_orth import  ( CustomCNN,PositionalEncoding,  init_layer) #( CustomCNN,  OrthogonalConv2d,PositionalEncoding, DoubleCNN, init_layer)
...
class EINV2(nn.Module):
    def __init__(self, cfg, dataset):
        ...
        self.sed_conv_block1 = nn.Sequential(
           CustomCNN(in_channels=4, out_channels=64),
            nn.AvgPool2d(kernel_size=(2, 2)),
           
        )
        self.sed_conv_block2 = nn.Sequential(
           CustomCNN(in_channels=64, out_channels=128),
            nn.AvgPool2d(kernel_size=(2, 2)),
        )
        self.sed_conv_block3 = nn.Sequential(
          CustomCNN(in_channels=128, out_channels=256),
            nn.AvgPool2d(kernel_size=(1, 2)),
        )
        self.sed_conv_block4 = nn.Sequential(
          CustomCNN(in_channels=256, out_channels=512),
            nn.AvgPool2d(kernel_size=(1, 2)),
        )

        self.doa_conv_block1 = nn.Sequential(
            CustomCNN(in_channels=self.in_channels, out_channels=64),
            nn.AvgPool2d(kernel_size=(2, 2)),
        )
