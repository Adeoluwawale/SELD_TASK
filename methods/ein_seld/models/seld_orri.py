...
from methods.utils.model_utilities import(PositionalEncoding, init_layer)
from methods.utils.model_utilities_wale import (DoubleConv)                                    
class EINV2(nn.Module):
    def __init__(self, cfg, dataset):
        ...
        self.sed_conv_block1 = nn.Sequential(
            DoubleConv(in_channels= 4, out_channels= 64), #in_channels
            nn.AvgPool2d((2, 2)),
        )
        self.sed_conv_block2 = nn.Sequential(
            DoubleConv(in_channels= 64, out_channels= 128),
            # _ResnetBasicBlock(inplanes=64, planes=128),
            nn.AvgPool2d((2, 2)),
        )
        ...
        self.sed_conv_block5 = nn.Sequential(
            DoubleConv(in_channels=512,out_channels=1024),
            nn.AvgPool2d((1, 2))
        ) 

        self.sed_conv_block6 = nn.Sequential(
            DoubleConv(in_channels=1024, out_channels=2048),
            nn.AvgPool2d((1, 2))
        )    
        ...
        self.doa_conv_block5 = nn.Sequential(
            DoubleConv(in_channels=512,out_channels=1024),
            nn.AvgPool2d((1, 2))
        )   
        self.doa_conv_block6 = nn.Sequential(
            DoubleConv(in_channels=1024, out_channels=2048),
            nn.AvgPool2d((1, 2))
        )    
        self.stitch = nn.ParameterList([
            ...
            #nn.Parameter(torch.FloatTensor(1024, 2, 2).uniform_(0.1, 0.9))
        ])
        ...
        self.fc_sed_track1 = nn.Linear(2048, 14, bias=True) #512
        self.fc_sed_track2 = nn.Linear(2048, 14, bias=True)
        self.fc_doa_track1 = nn.Linear(2048, 3, bias=True)
        self.fc_doa_track2 = nn.Linear(2048, 3, bias=True)
    ...
    def forward(self, x):
        ...
        x_sed = torch.einsum('c, nctf -> nctf', self.stitch[3][:, 0, 0], x_sed) + \
            torch.einsum('c, nctf -> nctf', self.stitch[3][:, 0, 1], x_doa)
        x_doa = torch.einsum('c, nctf -> nctf', self.stitch[3][:, 1, 0], x_sed) + \
            torch.einsum('c, nctf -> nctf', self.stitch[3][:, 1, 1], x_doa)
       
        x_sed = self.sed_conv_block5(x_sed)
        x_doa = self.doa_conv_block5(x_doa)
        x_sed = torch.einsum('c, nctf -> nctf', self.stitch[4][:, 0, 0], x_sed) + \
            torch.einsum('c, nctf -> nctf', self.stitch[4][:, 0, 1], x_doa)
        x_doa = torch.einsum('c, nctf -> nctf', self.stitch[4][:, 1, 0], x_sed) + \
            torch.einsum('c, nctf -> nctf', self.stitch[4][:, 1, 1], x_doa)
    
        x_sed = self.sed_conv_block6(x_sed)
        x_doa = self.doa_conv_block6(x_doa)
