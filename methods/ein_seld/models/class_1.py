...
class FrequencyModel(nn.Module):
    ...
    def __init__(
        ...
        #self.m = torchaudio.create_model(self.model_name, pretrained=self.pretrained, num_classes=output)
       # def forward(self,x):
        
           # out=self.m(x)

           # return out
def change_first_layer(sed):  #m
  for conv_block1, child in m.named_children():
    if isinstance(child, nn.Conv2d):
      ...
      sed._modules[conv_block1] = nn.Conv2d(4, 64)#**kwargs)
      return True
    else:
