...
class ConvBlock(nn.Module):
    ...
    def change_first_layer(Cnn14, self):
        for name, child in Cnn14.named_children():
              ...
              Cnn14._modules[self.conv_block1] = ConvBlock(4, **kwargs)
              return True
        else:
...
class Transfer_Cnn14(nn.Module):
    def __init__(self, in_channels_sed,  classes_num, freeze_base, 
                ): # sample_rate, window_size, hop_size, mel_bins, fmin, fmax
        ...
        self.base = Cnn14(classes_num)  # sample_rate, window_size, hop_size, mel_bins, fmin, fmax
        #self.conv_block1 = Cnn14.ConvBlock
        in_channels_sed = 4
        in_channel_doa = 7
        # Transfer to another task layer
       #self.fc_transfer = nn.Linear(2048, classes_num, bias=True)
        self.conv_block1 = ConvBlock(4, 64)
        Transfer_Cnn14_sed= self.conv_block1
        Transfer_Cnn14_sed
        if freeze_base:
    ...
    def load_from_pretrain(self, pretrained_checkpoint_path):
        checkpoint = torch.load('/mnt/raid/ni/WALE_SEld/EIN-SELD/Cnn14_DecisionLevelMax_mAP=0.385.pth')# pretrained_checkpoint_path
